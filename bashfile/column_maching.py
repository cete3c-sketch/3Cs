...
def normalize_cluster(df):
    df = df.copy()
    # Convert all column names to lowercase (for matching only)
    lower_cols = [c.lower().strip() for c in df.columns]

    # Define common variants of "cluster number"
    alt_names = ["CLUSTER NUMBER"]

    # Find if any column matches one of the variants
    for i, col in enumerate(lower_cols):
        if col in alt_names:
            df = df.rename(columns={df.columns[i]: "cluster number"})
            break
